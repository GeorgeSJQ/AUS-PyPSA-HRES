...
def get_generator_capacities(n, component_type="Generator"):
    """
    Extract optimized capacities for all generators or storage units.
    ...
    component_type : str
        The component type to extract - "Generator" or "StorageUnit"
    
    Returns:
    --------
    pd.DataFrame
        DataFrame with optimized capacities
    """
    if component_type not in ["Generator", "StorageUnit"]:
        raise ValueError("component_type must be either 'Generator' or 'StorageUnit'")
    
    capacities = pd.DataFrame(n.df(component_type).p_nom_opt)
    capacities.columns = ['Capacity (MW)']
    
    # Add build year and carrier information
    capacities['Build Year'] = n.df(component_type).build_year
    capacities['Carrier'] = n.df(component_type).carrier
    capacities['Lifetime'] = n.df(component_type).lifetime
    
    return capacities

def get_energy_generation_by_period(n):
    # Initialize empty DataFrame to store results
    periods = sorted(n.snapshots.get_level_values('period').unique())
    results = pd.DataFrame(index=n.generators.index, columns=periods)
    
    # Calculate generation for each period
    for period in periods:
        period_snapshots = n.snapshots[n.snapshots.get_level_values('period') == period]
        # Extract only the 'generators' column from the weightings
        weightings = n.snapshot_weightings.loc[period_snapshots, 'generators']
        
        # Make sure we're using proper broadcasting
        generation_df = n.generators_t.p.loc[period_snapshots]
        
        # Apply weightings one by one to avoid shape mismatch
        for col in generation_df.columns:
            # Use the generators column specifically
            weighted_gen = generation_df[col] * weightings.values
            results.loc[col, period] = weighted_gen.sum() / 1e3  # Convert to GWh
    
    # Add totals column
    results['Total (GWh)'] = results.sum(axis=1)
    
    # Add carrier information
    results['Carrier'] = n.generators.carrier
    
    return results

def get_storage_details(n):
    if len(n.storage_units) == 0:
        return {"message": "No storage units in the network"}
    
    # Power capacity
    power_capacity = pd.DataFrame(n.storage_units.p_nom_opt)
    power_capacity.columns = ['Power Capacity (MW)']
    
    # Energy capacity
    energy_capacity = pd.DataFrame(n.storage_units.p_nom_opt * n.storage_units.max_hours)
    energy_capacity.columns = ['Energy Capacity (MWh)']
    
    # Calculate utilization metrics
    periods = sorted(n.snapshots.get_level_values('period').unique())
    storage_dispatch = pd.DataFrame(index=n.storage_units.index, columns=periods)
    storage_charge = pd.DataFrame(index=n.storage_units.index, columns=periods)
    
    for period in periods:
        period_snapshots = n.snapshots[n.snapshots.get_level_values('period') == period]
        # Extract only the 'generators' column (or appropriate column)
        weightings = n.snapshot_weightings.loc[period_snapshots, 'stores'] 
        
        # Only keep positive values for dispatch
        dispatch = (n.storage_units_t.p.loc[period_snapshots] * weightings.values.reshape(-1, 1))
        dispatch = dispatch.clip(lower=0).sum() / 1e3  # Convert to GWh
        storage_dispatch.loc[:, period] = dispatch
        
        # Only keep negative values for charging and convert to positive
        charge = (n.storage_units_t.p.loc[period_snapshots] * weightings.values.reshape(-1, 1))
        charge = (-1) * charge.clip(upper=0).sum() / 1e3  # Convert to GWh and make positive
        storage_charge.loc[:, period] = charge
    
    # Add totals
    storage_dispatch['Total (GWh)'] = storage_dispatch.sum(axis=1)
    storage_charge['Total (GWh)'] = storage_charge.sum(axis=1)
    
    # Calculate cycles
    cycles = pd.DataFrame(index=n.storage_units.index)
    for unit in n.storage_units.index:
        energy_cap = energy_capacity.loc[unit, 'Energy Capacity (MWh)'] / 1e3  # Convert to GWh
        if energy_cap > 0:
            cycles.loc[unit, 'Equivalent Full Cycles'] = storage_charge.loc[unit, 'Total (GWh)'] / energy_cap
        else:
            cycles.loc[unit, 'Equivalent Full Cycles'] = 0
    
    # Additional metadata
    combined = power_capacity.join(energy_capacity)
    combined['Build Year'] = n.storage_units.build_year
    combined['Carrier'] = n.storage_units.carrier
    combined['Lifetime'] = n.storage_units.lifetime
    
    # Return a single DataFrame with all storage details
    result = pd.DataFrame(index=n.storage_units.index)
    result['Power Capacity (MW)'] = combined['Power Capacity (MW)']
    result['Energy Capacity (MWh)'] = combined['Energy Capacity (MWh)']
    result['Build Year'] = combined['Build Year']
    result['Carrier'] = combined['Carrier'] 
    result['Lifetime'] = combined['Lifetime']
    result['Total Discharge (GWh)'] = storage_dispatch['Total (GWh)']
    result['Total Charge (GWh)'] = storage_charge['Total (GWh)']
    result['Equivalent Full Cycles'] = cycles['Equivalent Full Cycles']
    
    return result

def get_capacity_by_investment_period(n, component_type="Generator"):
    """
    Show the active capacity of generators or storage units in each investment period.
    ...
    --------
    pd.DataFrame
        DataFrame with capacities by investment period
    """
    if component_type not in ["Generator", "StorageUnit"]:
        raise ValueError("component_type must be either 'Generator' or 'StorageUnit'")
    ...
    # Build capacity dataframe
    df = pd.concat(
        { 
            period: n.get_active_assets(component_type, period) * n.df(component_type).p_nom_opt
            for period in periods
        },
        axis=1
    )
    
    # Add carrier information for better grouping
    df['Carrier'] = n.df(component_type).carrier
...
def get_investment_period_summary(network: pypsa.Network) -> pd.DataFrame:
    """
    Generate a summary of key metrics by investment period.
    ...
    --------
    pd.DataFrame
        Summary with investment periods as columns and metrics as rows
    """
    
    if not hasattr(network, 'investment_periods'):
        print("Warning: Network does not have investment periods. Returning single period summary.")
        return pd.DataFrame()
    
    periods = sorted(network.investment_periods)
    summary = pd.DataFrame(index=[
        ...
        "Curtailment (GWh)",
        "Active Capacity - Solar (MW)",
        "Active Capacity - Wind (MW)",
        "Active Capacity - Battery (MW)",
        "Active Capacity - Gas (MW)"
    ], columns=periods)
    ...
    for period in periods:
        try:
            ...
            period_snapshots = network.snapshots[
                network.snapshots.get_level_values('period') == period
            ]
            
            # Generation by period
            period_gen = network.generators_t.p.loc[period_snapshots].sum().sum() / 1e3  # GWh
            summary.loc["Total Generation (GWh)", period] = period_gen
            
            # Load by period
            period_load = network.loads_t.p_set.loc[period_snapshots].sum().sum() / 1e3  # GWh
            summary.loc["Total Load (GWh)", period] = period_load
            
            # Average price
            if "electricity" in network.buses_t.marginal_price.columns:
                avg_price = network.buses_t.marginal_price.loc[period_snapshots, "electricity"].mean()
                summary.loc["Average Price ($/MWh)", period] = avg_price
            
            # Renewable share
            vre_gens = network.generators[network.generators.carrier.isin(['solar', 'wind', 'onwind', 'offwind'])]
            if len(vre_gens) > 0:
                vre_gen = network.generators_t.p.loc[period_snapshots, vre_gens.index].sum().sum() / 1e3
                if period_gen > 0:
                    vre_share = (vre_gen / period_gen) * 100
                    summary.loc["Renewable Share (%)", period] = vre_share
            
            # Storage discharge
            if not network.storage_units.empty:
                storage_dispatch = network.storage_units_t.p.loc[period_snapshots]
                discharge = storage_dispatch.clip(lower=0).sum().sum() / 1e3  # GWh
                summary.loc["Storage Discharge (GWh)", period] = discharge
            
            # Active capacities
            for tech in ['solar', 'wind', 'onwind']:
                tech_gens = network.generators[network.generators.carrier == tech]
                if len(tech_gens) > 0:
                    active_cap = network.get_active_assets("Generator", period)[tech_gens.index]
                    total_cap = (active_cap * network.generators.loc[tech_gens.index, 'p_nom_opt']).sum()
                    cap_name = "Wind" if tech in ['wind', 'onwind'] else tech.title()
                    summary.loc[f"Active Capacity - {cap_name} (MW)", period] = total_cap
            
            # Battery capacity
            battery_units = network.storage_units[network.storage_units.carrier.str.contains('battery', case=False, na=False)]
            if len(battery_units) > 0:
                active_battery = network.get_active_assets("StorageUnit", period)[battery_units.index]
                total_battery = (active_battery * network.storage_units.loc[battery_units.index, 'p_nom_opt']).sum()
                summary.loc["Active Capacity - Battery (MW)", period] = total_battery
            
            # Gas capacity  
            gas_gens = network.generators[network.generators.carrier.str.contains('gas', case=False, na=False)]
            if len(gas_gens) > 0:
                active_gas = network.get_active_assets("Generator", period)[gas_gens.index]
                total_gas = (active_gas * network.generators.loc[gas_gens.index, 'p_nom_opt']).sum()
                summary.loc["Active Capacity - Gas (MW)", period] = total_gas
                
        except Exception as e:
            print(f"Warning: Could not calculate metrics for period {period}: {e}")
            continue
    
    return summary


def export_detailed_results(network: pypsa.Network, 
                           output_dir: str = "results",
                           save_format: str = "csv") -> Dict[str, pd.DataFrame]:
    """
    Export detailed results to files and return dictionary of DataFrames.
    ...
    save_format : str, default "csv"
        Format to save files ("csv" or "excel")
    ...
    overview = generate_multiperiod_overview(network)
    results["overview"] = overview.to_frame("Value")
    
    # 2. Investment period summary
    if hasattr(network, 'investment_periods'):
        period_summary = get_investment_period_summary(network)
        results["period_summary"] = period_summary
    
    # 3. Generator capacities and details
    if not network.generators.empty:
        gen_capacities = pd.DataFrame({
            'Optimal_Capacity_MW': network.generators.p_nom_opt,
            'Carrier': network.generators.carrier,
            'Build_Year': getattr(network.generators, 'build_year', 'N/A'),
            'Lifetime': getattr(network.generators, 'lifetime', 'N/A')
        })
        results["generator_capacities"] = gen_capacities
    
    # 4. Storage unit details
    if not network.storage_units.empty:
        storage_details = pd.DataFrame({
            'Power_Capacity_MW': network.storage_units.p_nom_opt,
            'Energy_Capacity_MWh': network.storage_units.p_nom_opt * network.storage_units.max_hours,
            'Carrier': network.storage_units.carrier,
            'Build_Year': getattr(network.storage_units, 'build_year', 'N/A'),
            'Lifetime': getattr(network.storage_units, 'lifetime', 'N/A')
        })
        results["storage_details"] = storage_details
    
    # 5. Component statistics from PyPSA
    try:
        stats = network.statistics()
        results["component_statistics"] = stats
    except Exception as e:
        print(f"Warning: Could not get component statistics: {e}")
